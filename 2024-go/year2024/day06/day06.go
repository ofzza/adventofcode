...
// Implementation
func (day Day06) Run (index int, input any, verbose bool) (any, string, error) {
	// Initialize
	var output = ""
	...
	var guard = struct { x int; y int; direction byte }{ direction: pathing.DirectionTop }
	...
	if index == 1 {
		...
		// Trace guard's path
		if verbose { output += fmt.Sprintf("Bounds: X[%v, %v] Y[%v, %v]\n", boundsHorizontal[0], boundsHorizontal[1], boundsVertical[0], boundsVertical[1]) }
		tracePath(guard, boundsHorizontal, boundsVertical, obstaclesXY, obstaclesYX, func (direction byte, length int, a []int, b []int) bool {
			// Output section
			var label string
			if direction == pathing.DirectionRight 	{ label	= "RIGHT" }
			if direction == pathing.DirectionLeft  	{ label	= "LEFT " }
			if direction == pathing.DirectionTop 	 	{ label	= "UP   " }
			if direction == pathing.DirectionBottom	{ label	= "DOWN " }
			if verbose { output += fmt.Sprintf("- Moving %v ([%v, %v] -> [%v, %v]) -> %v: ", label, a[0], a[1], b[0], b[1], length) }
			// Note all traveled positions
			var section = make([][]int, 0, length)
			if direction == pathing.DirectionRight 	{ for x:=a[0]; x<=b[0]; x++ { section = append(section, []int { x, a[1] }) } }
			if direction == pathing.DirectionLeft  	{ for x:=a[0]; x>=b[0]; x-- { section = append(section, []int { x, a[1] }) } }
			if direction == pathing.DirectionTop 	 	{ for y:=a[1]; y>=b[1]; y-- { section = append(section, []int { a[0], y }) } }
			if direction == pathing.DirectionBottom	{ for y:=a[1]; y<=b[1]; y++ { section = append(section, []int { a[0], y }) } }
			for _, position := range section {
				var index, err = indexer.CoordinatesToIndex(position)
				if err == nil {
					traversed[index] = true
					if verbose { output += fmt.Sprintf("%v, ", position) }
				}
			}
			if verbose { output += fmt.Sprintln() }
			return true
		})
	...
	if index == 2 {
		...
		for y:=boundsVertical[0]; y<=boundsVertical[1]; y++ {
			for x:=boundsHorizontal[0]; x<=boundsHorizontal[1]; x++ {
				// Prompt
				if verbose { output += fmt.Sprintf("- Additional obstacle: [%v, %v] -> Starting from [%v, %v]\n", x, y, guard.x, guard.y) }
				// fmt.Printf("- Additional obstacle: [%v, %v] -> Starting from [%v, %v]\n", x, y, guard.x, guard.y)
				// Create additional obstacle
				var updatedObstaclesXY = make(map[int][]int)
				for key, value := range obstaclesXY { updatedObstaclesXY[key] = append([]int {}, value...) }
				var updatedObstaclesYX = make(map[int][]int)
				for key, value := range obstaclesYX { updatedObstaclesYX[key] = append([]int {}, value...) }
				...
				if okObstacle {
					if verbose { output += "  ~~~> SKIP: Already obstacle!\n" }
					continue
				}
				...
				var loopDetectionTraceFinished = tracePath(guard, boundsHorizontal, boundsVertical, updatedObstaclesXY, updatedObstaclesYX, func (direction byte, length int, a []int, b []int) bool {
					...
					if direction == pathing.DirectionTop 	 	{ label	= "UP   " }
					if direction == pathing.DirectionBottom	{ label	= "DOWN " }
					if verbose { output += fmt.Sprintf("  - Checking %v ([%v, %v] -> [%v, %v]) -> %v: ", label, a[0], a[1], b[0], b[1], length) }
					// Check all traveled positions for looping
					if (length < 0) { fmt.Println(length) }
					var section = make([][]int, 0, length)
					if direction == pathing.DirectionRight 	{ for x:=a[0]; x<=b[0]; x++ { section = append(section, []int { x, a[1] }) } }
					if direction == pathing.DirectionLeft  	{ for x:=a[0]; x>=b[0]; x-- { section = append(section, []int { x, a[1] }) } }
					if direction == pathing.DirectionTop 	 	{ for y:=a[1]; y>=b[1]; y-- { section = append(section, []int { a[0], y }) } }
					...
					for _, position := range section {
						...
						if err == nil {
							...
							// Log position
							if verbose { output += fmt.Sprintf("%v(%v), ", position, p) }
							// Set/Update position direction(s)
							if !ok { traversed[index] = direction } else { traversed[index] = p | direction }
							// Check if loop detected
							if ok && (p & direction) != 0 {
								if verbose { output += " ~~~> LOOP DETECTED!\n" }
								return false
							}
						}
					}
				...
				if !loopDetectionTraceFinished {
					...
					if verbose { output += "    ~~~> no loop detected\n" }
				}
			}
		}
	...
	if index == -1 {
		...
		tracePath(guard, boundsHorizontal, boundsVertical, obstaclesXY, obstaclesYX, func (direction byte, length int, a []int, b []int) bool {
			...
			if direction == pathing.DirectionTop 	 	{ label	= "UP   " }
			if direction == pathing.DirectionBottom	{ label	= "DOWN " }
			if verbose { output += fmt.Sprintf("- Moving %v ([%v, %v] -> [%v, %v]) -> %v:\n", label, a[0], a[1], b[0], b[1], length) }
			// Note all traveled positions
			var section = make([][]int, 0, length)
			if direction == pathing.DirectionRight 	{ for x:=a[0]; x<=b[0]; x++ { section = append(section, []int { x, a[1] }) } }
			if direction == pathing.DirectionLeft  	{ for x:=a[0]; x>=b[0]; x-- { section = append(section, []int { x, a[1] }) } }
			...
			for _, position := range section {
				...
				if direction == pathing.DirectionRight 	{ updatedObstacleCoords = []int { position[0] + 1, position[1] } }
				if direction == pathing.DirectionLeft 	{ updatedObstacleCoords = []int { position[0] - 1, position[1] } }
				if direction == pathing.DirectionTop 		{ updatedObstacleCoords = []int { position[0], position[1] - 1 } }
				if direction == pathing.DirectionBottom	{ updatedObstacleCoords = []int { position[0], position[1] + 1 } }
				// Check if obstacle can be placed: Within bounds
				if !indexer.CheckIfValidCoordinates(updatedObstacleCoords) { continue }
				// Check if obstacle can be placed: Not on start position
				if (updatedObstacleCoords[0] == guard.x && updatedObstacleCoords[1] == guard.y) { continue }
				// Check if obstacle can be placed: Not on top of existing obstacle
				var updateObstacleIndex, _= indexer.CoordinatesToIndex(updatedObstacleCoords)
				var _, okObstacle = obstacles[updateObstacleIndex]
				if okObstacle { continue }
				// Check if obstacle can be placed: Not on top of previously traversed path
				var _, okTraversed = traversed[updateObstacleIndex]
				if okTraversed { continue }
				// Place additional obstacle
				updatedObstaclesXY[updatedObstacleCoords[0]] = append(updatedObstaclesXY[updatedObstacleCoords[0]], updatedObstacleCoords[1])
				updatedObstaclesYX[updatedObstacleCoords[1]] = append(updatedObstaclesYX[updatedObstacleCoords[1]], updatedObstacleCoords[0])

				// Check for looping
				var loopingGuard = struct { x int; y int; direction byte }{ x: position[0], y: position[1], direction: direction }
				...
				var loopDetectionTraceFinished = tracePath(loopingGuard, boundsHorizontal, boundsVertical, updatedObstaclesXY, updatedObstaclesYX, func (direction byte, length int, a []int, b []int) bool {
					...
					for _, position := range section {
						...
						if err == nil {
							...
							if verbose { output += fmt.Sprintf("%v(%v), ", position, p) }
							// Set/Update position direction(s)
							if !ok { loopingPositions[index] = direction } else { loopingPositions[index] = p | direction }
							// Check if loop detected
							if ok && (p & direction) != 0 {
								if verbose { output += " ~~~> LOOP DETECTED!\n" }
								return false
				...
				var index, err = indexer.CoordinatesToIndex(position)
				if err == nil { traversed[index] = true }

				// Count loop
				if !loopDetectionTraceFinished { count++ }
...
func tracePath (
	guard struct { x int; y int; direction byte },
	...
	for {
		...
		if guard.direction & pathing.DirectionHorizontal != 0 {
			...
			if guard.direction == pathing.DirectionRight {
				var nextX int = -1
				for _, x := range inlinedObstacles { if x > guard.x && (nextX == -1 || x < nextX) { nextX = x } }
				if nextX != -1 { nextX -= 1 }
				if nextX != -1 {
					var length = nextX - guard.x
					if !pathSectionCallback(guard.direction, length, []int { guard.x, guard.y}, []int { nextX, guard.y }) { return false }
					guard.x = nextX
					guard.direction = pathing.DirectionBottom
				} else {
					var length = boundsHorizontal[1] - guard.x
					if !pathSectionCallback(guard.direction, length, []int { guard.x, guard.y }, []int { boundsHorizontal[1], guard.y }) { return false }
					break
				}
			} else
			// Left
			if guard.direction == pathing.DirectionLeft {
				...
				if nextX != -1 {
					...
					if !pathSectionCallback(guard.direction, length, []int { guard.x, guard.y }, []int { boundsHorizontal[0], guard.y }) { return false }
					break
				}
			}
		} else
		...
		if guard.direction & pathing.DirectionVertical != 0 {
			...
			if guard.direction == pathing.DirectionTop {
				var nextY int = -1
				for _, y := range inlinedObstacles { if y < guard.y && (nextY == -1 || y > nextY) { nextY = y } }
				if nextY != -1 { nextY += 1 }
				if nextY != -1 {
					var length = guard.y - nextY
					if !pathSectionCallback(guard.direction, length, []int { guard.x, guard.y }, []int { guard.x, nextY }) { return false }
					guard.y = nextY
					guard.direction = pathing.DirectionRight
				} else {
					var length = guard.y - boundsVertical[0]
					if !pathSectionCallback(guard.direction, length, []int { guard.x, guard.y }, []int { guard.x, boundsVertical[0] }) { return false }
					break
				}
			} else
			// Bottom
			if guard.direction == pathing.DirectionBottom {
				...
				if nextY != -1 {
					...
					if !pathSectionCallback(guard.direction, length, []int { guard.x, guard.y }, []int { guard.x, boundsVertical[1] }) { return false }
					break
				}
			}
		}
