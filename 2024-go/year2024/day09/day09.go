...
func (day Day09) Run (index int, input any, verbose bool) (any, string, error) {
	...
	for i, r := range line {
		...
		if id != -1 {
			...
			for j:=0; j<size; j++ { diskFs = append(diskFs, Block{ id: -1, size: 0 }) }
		}
	}
	...
	if index == 1 {
		...
		for i:=0; i<len(diskImage); i++ {
			if diskImage[i] != -1 { cksum += i * diskImage[i] }
		}
	...
	if index == 2 {
		...
		// Echo disk image
		if verbose { output += "- FS: " + echoDiskFs(diskFs) }

		// Compact
		for i:=(len(diskFs) - 1); i>=0; i-- {
			// Check if current position contains a data
			if diskFs[i].id == -1 { continue }
			// Echo FS record being compacted			
			if verbose { output += fmt.Sprintf("- Compacting block id=%v, size=%v\n", diskFs[i].id, diskFs[i].size) }
			...
			if emptyPointer == -1 { continue; }
			if emptyPointer >= i { continue; }
			// Echo FS empty space found
			if verbose { output += fmt.Sprintf("  - Found empty space at index=%v, size=%v\n", emptyPointer, diskFs[emptyPointer].size) }
			// Move block
			if diskFs[emptyPointer].size >= diskFs[i].size {
				// Check for pre-alocated empty space extension block(s)
				if diskFs[emptyPointer + 1].id != -1 || diskFs[emptyPointer + 1].size != 0 { return 0, output, errors.New("(pre)alocated empty space not found") }				
...
func echoDiskImage (diskImage []int) string {
	...
	// Output disk image
	for i:=0; i<len(diskImage); i++ {
		if diskImage[i] != -1 { output += fmt.Sprintf("%v", diskImage[i]) } else { output += "." }
	}
	output += "\n"
	// Return output
	return output
...
func generateDiskImage (diskFs []Block) []int {
	// Calculate size
	var size int = 0
	for i:=0; i<len(diskFs); i++ { size += diskFs[i].size }
	// Allocate disk image
