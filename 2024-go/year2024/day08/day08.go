...
// Implementation
func (day Day08) Run (index int, input any, verbose bool) (any, string, error) {
	// Initialize
	var output = ""
	...
	for _, l := range lines { for _, r := range l { array = append(array, r) } }
	...
	for y:=0; y<dimensions[1]; y++ {
		for x:=0; x<dimensions[0]; x++ {
			...
			if (frequency != '.') {
				...
				if !ok { freqAntennas[frequency] = [][]int { } }
	...
	if index == 1 {
		...
		iterateAntennaPairs(freqAntennas, func (frequency rune, coordsA []int, coordsB []int) {
			// Echo antenna pair
			if verbose { output += fmt.Sprintf("  - %v -> %v: ", coordsA, coordsB) }
			// Get antinodes
			...
			// Echo antinodes
			if verbose { output += fmt.Sprintf("%v\n", anodesArray) }
			...
			for _, anode := range anodesArray {
					...
					if !okIndex { anodes[index] = true; count++ }
			}
		})
...
func findAntinodes (indexer matrix.MatrixIndexer, coordsA []int, coordsB []int, limit int) [][]int {
	...
	for {
		...
		if limit > 0 && len(anodes) >= limit { break }
		// Calculate antinode
		var anode = []int { node[0] + diff[0], node[1] + diff[1] }
		// Check if out-of-bounds
		if !indexer.CheckIfValidCoordinates(anode) { break }
		// Store antinode
