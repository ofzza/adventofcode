...
// Implementation
func (day Day02) Run (index int, input any, verbose bool) (any, string, error) {
	// Initialize
	var output = ""
	...
	if index == 1 {
		...
		for reportIndex, report := range reports {
			...
			if unsafeIndices != nil {
				if (verbose) { output += fmt.Sprintf("- Unsafe report #%v %v: %v\n", reportIndex, report, unsafeOutput) }
			} else 
			// Count as safe
			{ safe++ }
		}
		...
		for reportIndex, report := range reports {
			...
			if unsafeIndices != nil {
				...
				for _, unsafeIndex := range unsafeIndices {
					...
					if unsafeIndices != nil {
						if (verbose) { output += fmt.Sprintf("  - Unsafe after retry having dropped #%v %v: %v\n", unsafeIndex, removed, unsafeOutput) }
					} else
					// Count as safe after retry
					{
						safe++;
						if (verbose) { output += fmt.Sprintf("  ... SAFE after retry having dropped #%v %v!\n", unsafeIndex, removed) }
						continue report
					}
				}
			} else 
...
func findUnsafe(report []int, verbose bool) ([]int, string) {
	...
	for i:=1; i<len(report); i++ {
		...
		if report[i - 1] == report[i] {
			if (verbose) { output += fmt.Sprintf("Level: [%v %v]", report[i - 1], report[i]); }
		...
		if (!asc && (report[i - 1] - report[i] < 1 || report[i - 1] - report[i] > 3)) {
			if (verbose) { output += fmt.Sprintf("Diff <1 || >3: %v - %v = %v", report[i - 1], report[i], (report[i - 1] - report[i])); }
			return []int{i - 1, i}, output;
		}
		if (asc && (report[i] - report[i - 1] < 1 || report[i] - report[i - 1] > 3)) {
			if (verbose) { output += fmt.Sprintf("Diff <1 || >3: %v - %v = %v", report[i], report[i - 1], (report[i] - report[i - 1])); }
			return []int{i - 1, i}, output;
		}
		// Count asc/desc
		if asc { ascs = append(ascs, []int{ i - 1, i }) } else { descs = append(descs, []int{ i - 1, i }) }
		if len(ascs) > 0 && len(descs) > 0 {
			if (verbose) { output += fmt.Sprintf("Inflection: [%v %v]", report[i - 1], report[i]); }
