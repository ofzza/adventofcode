...
func (day Day14) Run (index int, input any, verbose bool) (any, string, error) {
	...
	if !okWidth || !okHeight || !okDuration || !okValue { return nil, output, errors.New("failed casting execution to correct Input/Output types") }
	...
	if index == 1 {
		...
		if verbose {
			...
			for _, robot := range robots { output += fmt.Sprintf("  - Robot: position=%v velocity=%v\n", robot.position, robot.velocity) }
		}
		...
		for i:=0; i<4; i++ { quadrants = append(quadrants, 0) }
		for _, robot := range updated {
			// Echo
			if verbose { output += fmt.Sprintf("  - Robot: position=%v velocity=%v", robot.position, robot.velocity) }
			// Add to correct quadrant
			if robot.position[0] < midX && robot.position[1] < midY { quadrants[0]++; if verbose { output += fmt.Sprintln(" (Q1)") } } else
			if robot.position[0] > midX && robot.position[1] < midY { quadrants[1]++; if verbose { output += fmt.Sprintln(" (Q2)") } } else
			if robot.position[0] < midX && robot.position[1] > midY { quadrants[2]++; if verbose { output += fmt.Sprintln(" (Q3)") } } else
			if robot.position[0] > midX && robot.position[1] > midY { quadrants[3]++; if verbose { output += fmt.Sprintln(" (Q4)") } } else 
	...
	if index == 2 {
		...
		for i:=0; true; i++ {
			...
			for _, r := range updated {
				if r.position[0] > 10 && r.position[0] < width - 10 && r.position[1] > 10 && r.position[1] < height - 10 { count++ }
 			}
			...
			if detected && verbose {
				...
				for y:=0; y<height; y++ {
					for x:=0; x<width; x++ {
						...
						var count int = 0
						for _, r := range updated { if y == r.position[1] && x == r.position[0] { count++ } }
						// Output number of robots at this position
						if count == 0 { output += fmt.Sprintf("%c", '.') } else { output += fmt.Sprintf("%c", '#') }
					}
					output += fmt.Sprintln()
				}
				output += fmt.Sprintln()
...
func update (robots []Robot, width int, height int, duration int) []Robot {
	...
	for _, r := range robots {
		...
		if updatedX < 0 { updatedX = width + updatedX }
		var updatedY = (r.position[1] + r.velocity[1] * duration) % height
		if updatedY < 0 { updatedY = height + updatedY }
