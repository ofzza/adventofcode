...
func (indexer *MatrixIndexer) initialize () {
	...
	for i:=0; i<len(indexer.dimensions); i++ {
		...
		for i := range coords { coords[i] = append(coords[i], 0) }
	...
	for _, coord := range coords {
		...
		if (slices.Contains(coord, 0)) { indexer.relativeNeighborCoordsWithoutDiagonals = append(indexer.relativeNeighborCoordsWithoutDiagonals, coord) }
	}
}
...
func (indexer *MatrixIndexer) IndexToCoordinates (index int) ([]int, error) {
	...
	if index >= 0 { normalizedIndex = index % indexer.length } else { normalizedIndex = ((-1 * ((-1 * index) % indexer.length)) + indexer.length) % indexer.length }
...
func (indexer *MatrixIndexer) CoordinatesToIndex (coords []int) (int, error) {
	if !indexer.CheckIfValidCoordinates(coords) { return -1, errors.New("invalid coordinates provided") }
...
func (indexer *MatrixIndexer) GetNeighboringCoordinatesForCoords (coords []int, diagonal bool) ([][]int, error) {
	...
	for _, relative := range relatives {
		...
		if valid { neighbors = append(neighbors, neighbor) }
	}
