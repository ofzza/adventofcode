...
// Run a puzzle test
module.exports.test = (puzzle, args, { example = true, expected, transform = r => r, render } = {}) => {
  // Run test
  ...
  if (!colors) {
    ...
    console.log(`  ... ${ example ? 'EXAMPLE: ' : 'MY INPUT:'.yellow }`
                ...
                + ` => ${ stringify(transform(result)) } ${ !error ? (expected !== undefined ? '[OK]'.green : '') : '[ERROR]'.red }`
...
module.exports.logProgress = (output) => {
  if (this.flags.PROGRESS) {
    console.log(colors ? output.green : output);
  }
};
